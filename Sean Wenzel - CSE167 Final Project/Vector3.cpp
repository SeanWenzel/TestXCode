...
float Vector3::getElement(int i) {
    return m[i];
}
...
Vector3 Vector3::add(Vector3 a)
{
    ...
    // Add the corresponding components of the calling Vector3 with the Vector3 "a"
    b.m[0] = m[0] + a.m[0];
    b.m[1] = m[1] + a.m[1];
    b.m[2] = m[2] + a.m[2];
...
Vector3 Vector3::subtract(Vector3 a)
{
    ...
    // Subtract the corresponding components of the calling Vector3 with the Vector3 "a"
    b.m[0] = m[0] - a.m[0];
    b.m[1] = m[1] - a.m[1];
    b.m[2] = m[2] - a.m[2];
...
Vector3 Vector3::negate(void)
{
    ...
    // Negate each component of the calling Vector3
    b.m[0] = -m[0];
    b.m[1] = -m[1];
    b.m[2] = -m[2];
...
{
    // Calculate the dot product between the calling Vector3 and the Vector3 "a"
...
Vector3 Vector3::cross(Vector3 a)
{
    ...
    // Computer the cross product between the calling Vector3 and the Vector3 "a"
...
{
    // Calculate and return length of Vector3
    return pow(pow(m[0], 2) + pow(m[1], 2) + pow(m[2], 2), 0.5);
}
...
Vector3 Vector3::normalize(void)
{
    ...
    // ASK ABOUT THE ZERO LENGTH CASE
    if (mag == 0.0) {
        b.m[0] = 0.0;
        b.m[1] = 0.0;
        b.m[2] = 0.0;
    ...
    // Divide each component by the Vector3's magnitude
    else {
        b.m[0] = m[0]/mag;
        b.m[1] = m[1]/mag;
        b.m[2] = m[2]/mag;
